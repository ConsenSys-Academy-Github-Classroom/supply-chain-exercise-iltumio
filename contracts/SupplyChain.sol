...
contract SupplyChain {
  ...
  // <modifier: isOwner
  modifier isOwner() {
    require(msg.sender == owner, "This function is callable only by the owner");
    _;
  }
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].state == State.ForSale 
      && items[_sku].seller != address(0), "The item is not on sale");
    _;
  }
  // modifier sold(uint _sku) 
  modifier sold(uint _sku) {
    require(items[_sku].state == State.Sold, "The item is not in sold state");
    _;
  }
  // modifier shipped(uint _sku) 
  modifier shipped(uint _sku) {
    require(items[_sku].state == State.Shipped, "The item is not in shipped state");
    _;
  }
  // modifier received(uint _sku) 
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, "The item is not in received state");
    _;
  }
  ...
  constructor() public {
    ...
    owner = msg.sender;
    // 2. Initialize the sku count to 0. Question, is this necessary? not necessary
  }
